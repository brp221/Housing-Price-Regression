...
#_______________________________________WRANGLING_______________________________________
import missingno as msno 
from sklearn.preprocessing import LabelEncoder
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA

cols_2_remove = [col for col in original_train_df.columns if original_train_df[col].isna().sum()/original_train_df.shape[0]   > 0.6 ] # rid columsn where more than 60% data is missing
cols_2_impute = [col for col in original_train_df.columns if 0 < original_train_df[col].isna().sum()/original_train_df.shape[0]   < 0.6   ] # impute other columsn with missing data 
columns_2_keep = list(set(original_train_df.columns) - set(cols_2_remove))
train_df = original_train_df[columns_2_keep]   
...
remaing_cols = ['LotFrontage', 'LotArea', 'OverallQual', 'OverallCond', 'YearBuilt', 'YearRemodAdd', 'MasVnrArea', 'BsmtFinSF1', 'BsmtFinSF2', 
                ...
                'MiscVal', 'MoSold', 'YrSold', 'MSSubClass', 'GarageArea', 'Id', 'SalePrice', 'KitchenAbvGr', 'BedroomAbvGr', 'TotalBsmtSF' ]

print('did we forget any columns?  --> ', len(set(list(one_hot_cols + nominal_cols + ordinal_cols + remaing_cols) )) != len(train_df.columns) )
print('columns forgotten', set(train_df.columns) - set(list(one_hot_cols + nominal_cols + ordinal_cols + remaing_cols) ))
print('columns repeated', set(list(one_hot_cols + nominal_cols + ordinal_cols + remaing_cols) ) - set(train_df.columns)   )

#   1 Hot Encoder
one_hot_df_copy = train_df[one_hot_cols+['Id']]
...
#   LabelEncoding
label_df = train_df[nominal_cols+['Id']].copy()
...
#   OrdinalEncoding
ordinal_df = train_df[ordinal_cols+remaing_cols]
...
train_df = one_hot_df.merge(ordinal_df, how="outer", on="Id")
train_df = train_df.merge(label_df, how="outer", on="Id")
print(train_df.info())
...
train_mice_df = train_df.copy(deep=True)
train_mice_df.iloc[:, :] = mice_imputer.fit_transform(train_mice_df)

# KNN IMPUTATION
knn_imputer = KNN()
train_knn_df = train_df.copy(deep=True)
train_knn_df.iloc[:, :] = knn_imputer.fit_transform(train_knn_df)

# we will run model on each resulting imputed df. 
 
#_______________________________________PCA_______________________________________

pca_subset_df = train_mice_df[remaing_cols]
...
mask = np.triu(np.ones_like(train_knn_df.corr().abs(), dtype=bool))
dataset_corr = sns.heatmap(train_knn_df.corr().abs(), mask=mask, center=0, cmap=cmap, linewidths=1, annot=True, fmt='.2f')
...
train_mice_reduced_df = train_mice_df.drop(columns=drop_candidates)
train_knn_reduced_df = train_knn_df.drop(columns=drop_candidates)
...
train_mice_reduced_df.to_csv('wrangled_data/train_mice_reduced_df.csv', index=False, )
